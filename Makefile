...
$(BUILD_DIR)/bootloader.elf: $(BOOTLOADER_DIR)/boot.S $(BOOTLOADER_DIR)/main.c $(KERNEL_DIR)/kernel.c $(KERNEL_DIR)/memory.c $(KERNEL_DIR)/timer.c $(KERNEL_DIR)/interrupt.c $(KERNEL_DIR)/scheduler.c $(KERNEL_DIR)/task.c $(LIBS_DIR)/uart.c $(DRIVERS_DIR)/framebuffer.c $(DRIVERS_DIR)/virtio_gpu.c $(UI_DIR)/window.c $(BOOTLOADER_DIR)/linker.ld
	...
	$(LD) -T $(BOOTLOADER_DIR)/linker.ld $(BUILD_DIR)/boot.o $(BUILD_DIR)/main.o $(BUILD_DIR)/kernel.o $(BUILD_DIR)/memory.o $(BUILD_DIR)/timer.o $(BUILD_DIR)/interrupt.o $(BUILD_DIR)/scheduler.o $(BUILD_DIR)/task.o $(BUILD_DIR)/uart.o $(BUILD_DIR)/framebuffer.o $(BUILD_DIR)/virtio_gpu.o $(BUILD_DIR)/window.o -o $(BUILD_DIR)/bootloader.elf

# Build kernel
kernel: $(BUILD_DIR)/kernel.elf

$(BUILD_DIR)/kernel.elf: $(KERNEL_DIR)/kernel.c $(KERNEL_DIR)/memory.c $(KERNEL_DIR)/timer.c $(KERNEL_DIR)/interrupt.c $(KERNEL_DIR)/scheduler.c $(KERNEL_DIR)/task.c $(LIBS_DIR)/uart.c $(DRIVERS_DIR)/framebuffer.c $(DRIVERS_DIR)/virtio_gpu.c $(UI_DIR)/window.c $(KERNEL_DIR)/linker.ld
...
run: build
	...
	$(QEMU) -M virt -cpu cortex-a53 -kernel $(BUILD_DIR)/bootloader.elf -device ramfb,width=1024,height=768 -serial stdio
