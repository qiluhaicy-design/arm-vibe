...
if grep -q "vibeOS Bootloader started" test_output.log && grep -q "vibeOS Kernel started" test_output.log && grep -q "Drawing text: vibeOS Test Screen" test_output.log && grep -q "Created window: Window 1" test_output.log; then
    echo "✓ Bootloader, kernel, framebuffer and window test messages found"
else
